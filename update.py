...
if __name__ == '__main__':
    ...
    model = load_model()
    model.load_weights('/content/models/model.15-0.89.hdf5')
    ...
    for i, image_name in enumerate(samples):
        ...
        text = ('Predict: {}, prob: {:.4}'.format(class_names[class_id][0][0], prob))
        results.append({'label': class_names[class_id][0][0], 'prob': '{:.4}'.format(prob)})
        # add predicted text to the image
        cv.putText(bgr_img, text, (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv.LINE_AA)
